...
#ifdef QC_ENABLE_HS
std::function<void( const std::uint32_t *, std::size_t )> SampleCL2DFlex::GetRunnableCallback()
{
    m_bOrchestratorEnabled = true;
    return std::bind( &SampleCL2DFlex::RunnableCallback, this, std::placeholders::_1,
                      std::placeholders::_2 );
}

void SampleCL2DFlex::RunnableCallback( const std::uint32_t *rids, std::size_t count )
{
    Execute();
}
#endif
...
void SampleCL2DFlex::Execute()
{
    ...
    if ( QC_STATUS_OK == ret )
    {
        ...
        if ( nullptr != bufferOutput )
        {
            ...
            {
                QC_ERROR( "CL2D execute failed for %" PRIu64 " : %d", frames.FrameId( 0 ), ret );
            }
        }
    }
#ifdef QC_ENABLE_HS
