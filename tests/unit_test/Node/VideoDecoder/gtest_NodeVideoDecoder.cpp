...
extern "C" {
  ...
  static bool g_emit_output_reconfig_once = false; // inject one reconfig before first OUTPUT_DONE
  static bool g_start_output_ioctl_should_fail = false; // Fail the device START ioctl when mode==VIDC_START_OUTPUT
  static std::atomic<int> g_fill_output_calls{0}; // Count output FILL IOCTLs to prove the loop broke before trying the second buffer
  ...
  int __mockup_device_close(ioctl_session_t* handle) { return 0; }
  ...
  int __mockup_device_ioctl(ioctl_session_t* handle,
                          uint32_t cmd,
                          ...
                          uint8_t* out, uint32_t out_len)
  {
    ...
    switch (cmd) {
      case VIDC_IOCTL_LOAD_RESOURCES:
        if (g_emit_load_done) EmitVidcEvent(VIDC_EVT_RESP_LOAD_RESOURCES); // → READY via PostInit
        return VIDC_ERR_NONE;

      case VIDC_IOCTL_RELEASE_RESOURCES:
        if (g_emit_release_done) EmitVidcEvent(VIDC_EVT_RESP_RELEASE_RESOURCES); // → INITIAL
        return VIDC_ERR_NONE;

      case VIDC_IOCTL_START: {
        if (in /*&& in_len >= sizeof(vidc_start_mode_type)*/) {
          auto mode = *reinterpret_cast<vidc_start_mode_type*>(in);
          if (mode == VIDC_START_INPUT)   { if (g_emit_start_input_done)  EmitVidcEvent(VIDC_EVT_RESP_START_INPUT_DONE); }
          else if (mode == VIDC_START_OUTPUT) {
              if (g_start_output_ioctl_should_fail) return VIDC_ERR_FAIL;
              if (g_emit_start_output_done) EmitVidcEvent(VIDC_EVT_RESP_START_OUTPUT_DONE); }
          else EmitVidcEvent(VIDC_EVT_RESP_START);
        } else {
      ...
      case VIDC_IOCTL_STOP: {
        if (g_emit_stop_done) {
          if (in /*&& in_len >= sizeof(vidc_stop_mode_type)*/) {
            ...
            if (mode == VIDC_STOP_INPUT)  EmitVidcEvent(VIDC_EVT_RESP_STOP_INPUT_DONE);
            else if (mode == VIDC_STOP_OUTPUT) EmitVidcEvent(VIDC_EVT_RESP_STOP_OUTPUT_DONE);
            else EmitVidcEvent(VIDC_EVT_RESP_STOP);
          } else {
      ...
      if (in /*&& in_len >= sizeof(vidc_frame_data_type)*/) {
          ...
          if (g_emit_output_reconfig_once) {
            ...
            EmitVidcEvent(VIDC_EVT_OUTPUT_RECONFIG); // triggers HandleOutputReconfig
          }
      ...
      case VIDC_IOCTL_SET_PROPERTY: {
        ...
        if (p->prop_hdr.prop_id == VIDC_I_BUFFER_REQUIREMENTS) {
          ...
          std::memcpy(&tmp, p->payload, std::min<uint32_t>(p->prop_hdr.size, sizeof(tmp)));
      ...
      case VIDC_IOCTL_GET_PROPERTY: {
        ...
        if (p->prop_hdr.prop_id == VIDC_I_BUFFER_REQUIREMENTS) {
          ...
          std::memcpy(out, &tmp, std::min<uint32_t>(out_len, static_cast<uint32_t>(sizeof(tmp))));
        }
        return VIDC_ERR_NONE;
      }
...
void VdTestDynamic( uint32_t bufferNum, QCImageFormat_e outFormat, const char *videoFile )
{
  ...
  switch( videoInfo.format ) {
    case QC_IMAGE_FORMAT_RGB888:   inFmt = "rgb888";     break;
    case QC_IMAGE_FORMAT_BGR888:   inFmt = "bgr888";     break;
    case QC_IMAGE_FORMAT_UYVY:     inFmt = "uyvy";       break;
    case QC_IMAGE_FORMAT_NV12:     inFmt = "nv12";       break;
    case QC_IMAGE_FORMAT_P010:     inFmt = "p010";       break;
    case QC_IMAGE_FORMAT_NV12_UBWC:inFmt = "nv12_ubwc";  break;
    case QC_IMAGE_FORMAT_TP10_UBWC:inFmt = "tp10_ubwc";  break;
    case QC_IMAGE_FORMAT_COMPRESSED_H264: inFmt = "h264"; break;
    case QC_IMAGE_FORMAT_COMPRESSED_H265: inFmt = "h265"; break;
    default:
  ...
  switch( outFormat ) {
    case QC_IMAGE_FORMAT_RGB888:   outFmt = "rgb888";     break;
    case QC_IMAGE_FORMAT_BGR888:   outFmt = "bgr888";     break;
    case QC_IMAGE_FORMAT_UYVY:     outFmt = "uyvy";       break;
    case QC_IMAGE_FORMAT_NV12:     outFmt = "nv12";       break;
    case QC_IMAGE_FORMAT_P010:     outFmt = "p010";       break;
    case QC_IMAGE_FORMAT_NV12_UBWC:outFmt = "nv12_ubwc";  break;
    case QC_IMAGE_FORMAT_TP10_UBWC:outFmt = "tp10_ubwc";  break;
    case QC_IMAGE_FORMAT_COMPRESSED_H264: outFmt = "h264"; break;
    case QC_IMAGE_FORMAT_COMPRESSED_H265: outFmt = "h265"; break;
    default:
      printf("error: unrecognized input format %s\n", inFmt);
      return;
  }
  ...
  for ( uint32_t i = 0; i < numOutputBufferReq; i++ )
  {
    ...
    ret = bufMgr.Allocate( ImageBasicProps( videoInfo.frameWidth, videoInfo.frameHeight,
                                           outFormat, QC_MEMORY_ALLOCATOR_DMA_VPU, QC_CACHEABLE ),
                           bufDesc );
    ASSERT_EQ( QC_STATUS_OK, ret );
...
struct VdUnitCounters { std::atomic<int> in_cb{0}, out_cb{0}, evt_cb{0}; };
...
protected:
  ...
  void SetUp() override {
    ...
    g_req_in  = {}; g_req_in.buf_type  = VIDC_BUFFER_INPUT;  g_req_in.actual_count  = 4; g_req_in.size  = 4096;
    g_req_out = {}; g_req_out.buf_type = VIDC_BUFFER_OUTPUT; g_req_out.actual_count = 4; g_req_out.size = 8192;
    ...
    dt.Set<uint32_t>("frameRate", 30);
    dt.Set<std::string>("inputImageFormat",  "h264"); // → VIDEO_CODEC_H264 in InitDrvProperty
    dt.Set<std::string>("outputImageFormat", "nv12");
    DataTree root; root.Set("static", dt);
    cfgJson = root.Dump();
  }
  ...
  static VideoFrameDescriptor_t MakeFrame(uint64_t handle, void* addr, uint32_t size,
                                          uint32_t w, uint32_t h, QCImageFormat_e fmt) {
    VideoFrameDescriptor_t f{};
    f.dmaHandle = handle; f.pBuf = addr; f.size = size; f.width = w; f.height = h; f.format = fmt; f.pid = 1234;
    return f;
  }
};
...
TEST_F(VideoDecoderTest, ValidateConfig_FailsOnUnsupportedFormats_And_OutOfRange)
{
  ...
  DataTree dt1; dt1.Set<std::string>("name","X"); dt1.Set<uint32_t>("id",1);
  dt1.Set<uint32_t>("width",128); dt1.Set<uint32_t>("height",128);
  dt1.Set<uint32_t>("frameRate",30); dt1.Set<bool>("bInputDynamicMode",true); dt1.Set<bool>("bOutputDynamicMode",true);
  dt1.Set<uint32_t>("numInputBufferReq",4); dt1.Set<uint32_t>("numOutputBufferReq",4);
  dt1.Set<std::string>("inputImageFormat","nv12");  // invalid as *compressed* input for decoder
  dt1.Set<std::string>("outputImageFormat","nv12");
  DataTree r1; r1.Set("static",dt1);
  ...
  DataTree dt2 = dt1; dt2.Set<std::string>("inputImageFormat","h264"); dt2.Set<std::string>("outputImageFormat","bgr888");
  DataTree r2; r2.Set("static",dt2); QCNodeInit_t i2{.config=r2.Dump(), .callback=TestOnDoneCb};
  EXPECT_NE(QC_STATUS_OK, dec.Initialize(i2));

  // 3) Buffers out of allowed range (min/max)
  DataTree dt3 = dt1; dt3.Set<std::string>("inputImageFormat","h264"); dt3.Set<std::string>("outputImageFormat","nv12");
  dt3.Set<uint32_t>("numInputBufferReq",  1); // too small
  dt3.Set<uint32_t>("numOutputBufferReq", 1); // too small
  DataTree r3; r3.Set("static",dt3); QCNodeInit_t i3{.config=r3.Dump(), .callback=TestOnDoneCb};
...
TEST_F(VideoDecoderTest, SubmitInputAndOutput_InvokesCallbacks)
{
  ...
  ASSERT_EQ(QC_STATUS_OK, dec.ProcessFrameDescriptor(inFd));   // → EMPTY_INPUT_BUFFER → RESP_INPUT_DONE
  ASSERT_EQ(QC_STATUS_OK, dec.ProcessFrameDescriptor(outFd));  // → FILL_OUTPUT_BUFFER → RESP_OUTPUT_DONE
...
TEST_F(VideoDecoderTest, OutputReconfig_Sequence_Completes_AndRemainsRunning)
{
  ...
  NodeFrameDescriptor outFd(QC_NODE_VIDEO_DECODER_OUTPUT_BUFF_ID + 1); outFd.Clear();
  ASSERT_EQ(QC_STATUS_OK, outFd.SetBuffer(QC_NODE_VIDEO_DECODER_OUTPUT_BUFF_ID, out));
  ASSERT_EQ(QC_STATUS_OK, dec.ProcessFrameDescriptor(outFd));   // HandleOutputReconfig → StartDriver(OUTPUT) → RESP_START_OUTPUT_DONE → FinishOutputReconfig
...
TEST_F(VideoDecoderTest, SubmitInputFrame_InvalidBufferRejected)
{
  ...
  NodeFrameDescriptor inFd(QC_NODE_VIDEO_DECODER_INPUT_BUFF_ID + 1); inFd.Clear();
...
TEST_F(VideoDecoderTest, SubmitOutputFrame_TooSmallAfterNegotiation_IsInvalid)
{
  ...
  NodeFrameDescriptor tmp(QC_NODE_VIDEO_DECODER_OUTPUT_BUFF_ID + 1); tmp.Clear();
...
static std::string BuildNonDynamicCfgJson(uint32_t w, uint32_t h,
                                          const char* inFmtStr, const char* outFmtStr,
                                          uint32_t numIn, uint32_t numOut)
{
  ...
  dt.Set<uint32_t>("frameRate", 30);
  dt.Set<bool>("bInputDynamicMode",  false);   // <-- non-dynamic so CheckBuffer() loops actually run
...
static void BuildBuffersVector(uint32_t w, uint32_t h,
                               QCImageFormat_e inFmt, QCImageFormat_e outFmt,
                               uint32_t numIn, uint32_t numOut,
                               ...
                               std::vector<std::reference_wrapper<QCBufferDescriptorBase_t>>& outRefs)
{
  ...
  outRefs.clear(); outRefs.reserve(numIn + numOut);

  // INPUT descriptors: size must be >= negotiated input bufSize (set by NegotiateBufferReq(INPUT)).
  // Our unit stub exposes g_req_in.size (default 4096 in SetUp()) so use at least that much.
...
static std::string BuildNonDynamicOutputCfgJson(uint32_t w, uint32_t h,
                                                const char* inFmtStr, const char* outFmtStr,
                                                uint32_t numIn, uint32_t numOut)
{
  DataTree dt;
...
static void BuildBuffersVectorForNonDynamicOutput(uint32_t w, uint32_t h,
                                                  QCImageFormat_e outFmt,
                                                  uint32_t numOut,
                                                  std::vector<VideoFrameDescriptor_t>& outObjs,
                                                  std::vector<std::reference_wrapper<QCBufferDescriptorBase_t>>& refs_out)
{
  outObjs.resize(numOut);
  refs_out.clear(); refs_out.reserve(numOut);
...
TEST_F(VideoDecoderTest, Initialize_NonDynamic_CallsCheckBuffer_OnOutput_FailsDueToFormat)
{
  ...
  // We'll deliberately provide OUTPUT descriptors with WRONG format (P010) so ValidateBuffer() fails,
  // which is called inside CheckBuffer(), proving the OUTPUT loop ran.
  ...
  //  - AllocateBuffer(OUTPUT) pushes m_outputBufferList; CheckBuffer(OUTPUT) loops and FAIL (format mismatch)
  auto rc = dec.Initialize(init);
  EXPECT_NE(QC_STATUS_OK, rc);                          // fails in OUTPUT CheckBuffer
  EXPECT_EQ(QC_OBJECT_STATE_ERROR, dec.GetState());     // Initialize marks ERROR on any CheckBuffer failure.
}
...
static std::string BuildCfgJson(
    uint32_t w, uint32_t h, uint32_t fps,
    const char* inFmtStr, const char* outFmtStr,
    bool inDyn, bool outDyn,
    uint32_t numIn, uint32_t numOut,
    const char* name = "ValidateCfg")
{
  DataTree dt;
...
TEST(Decoder_ValidateConfig, NullConfigPointer_FailsEarly)
{
  ...
  auto badCfg = BuildCfgJson(1280, 720, 30, "bogus_in_fmt", "bogus_out_fmt",
                             true, true, 4, 4, "NullCfg");
...
TEST_F(VideoDecoderTest, Initialize_NonDynamic_InputCheckBuffer_SizeZeroBranch)
{
  ...
  EXPECT_EQ(QC_OBJECT_STATE_ERROR, dec.GetState()); // Initialize marks ERROR on CheckBuffer failure
}
...
TEST_F(VideoDecoderTest, HandleOutputReconfig_SecondEventWhileInProgress_TakesInProgressBranch)
{
  ...
  //      negotiates output and calls StartDriver(OUTPUT) (which will later generate RESP_START_OUTPUT_DONE)
  EmitVidcEvent(VIDC_EVT_OUTPUT_RECONFIG);

  // 4) Immediately emit a second OUTPUT_RECONFIG before RESP_START_OUTPUT_DONE arrives.
  //    This forces HandleOutputReconfig() into: if (m_OutputReconfigInprogress) { ret = QC_STATUS_BAD_STATE; }
  EmitVidcEvent(VIDC_EVT_OUTPUT_RECONFIG);
...
TEST_F(VideoDecoderTest, FinishOutputReconfig_BreaksOnFirstSubmitFailure)
{
  ...
  // Build config: input dynamic; output non-dynamic so FinishOutputReconfig() will iterate m_outputBufferList
  auto cfgJson = BuildNonDynamicOutputCfgJson(W, H, /*in*/"h264", /*out*/"nv12",
                                              /*numIn*/4, /*numOut*/numOut);
  ...
  // Start → RUNNING so ValidateFrameSubmission will pass in SubmitOutputFrame  [3](https://qualcomm-my.sharepoint.com/personal/yabraham_qti_qualcomm_com/Documents/Microsoft%20Copilot%20Chat%20Files/VideoDecoder.cpp)
  ASSERT_EQ(QC_STATUS_OK, dec.Start());
  ASSERT_EQ(QC_OBJECT_STATE_RUNNING, dec.GetState());
  ...
  //  - FinishOutputReconfig(): loop over m_outputBufferList, first SubmitOutputFrame fails at CheckBuffer,
