...
static void Mockup_Cfg_Reset() { g_mock_cfg = MockupVidcConfig{}; }
...
static void EmitFrameEvent( uint32_t event_type, uint64_t handle, uint8_t *addr,
                            uint32_t size )
{
    if ( !g_cb.handler ) return;
    ...
    g_cb.handler( reinterpret_cast<unsigned char*>( &msg ), sizeof(msg),
                  g_cb.data );
}

// A trivially fast "sleep" so time-based waits don’t actually block
int __mockup_MM_Timer_Sleep(unsigned int /*msec*/) { return 0; }
                            ...
                            unsigned int in_len, unsigned char *out, unsigned int out_len )
{
    ...
    case VIDC_IOCTL_RELEASE_RESOURCES:
        ...
        if ( g_mock_cfg.emit_evt_release_done ) EmitEvent( VIDC_EVT_RESP_RELEASE_RESOURCES );
        return VIDC_ERR_NONE;
    ...
    case VIDC_IOCTL_GET_PROPERTY:
    {
        if (g_autofire_events) {
            ...
            auto* p = reinterpret_cast<vidc_drv_property_type*>(in);
            if (p->prop_hdr.prop_id == VIDC_I_BUFFER_REQUIREMENTS) {
                // Out payload expects vidc_buffer_reqmnts_type; copy from our store based on buf_type
                vidc_buffer_reqmnts_type tmp{};
    ...
    case VIDC_IOCTL_FILL_OUTPUT_BUFFER:
        if (!g_autofire_events) {
            ...
            {
                auto *fd = reinterpret_cast<vidc_frame_data_type *>( in );
                EmitFrameEvent( VIDC_EVT_RESP_OUTPUT_DONE, fd->frm_clnt_data, fd->frame_addr,
                                fd->data_len );
            }
        }
        return VIDC_ERR_NONE;
...
protected:
    ...
    void SetUp() override
    {
        ...
        ASSERT_EQ(QC_STATUS_OK,
                  dut.OpenDriver(&VidcDrvClientTest::InDoneCb,
                                 &VidcDrvClientTest::OutDoneCb,
                                 &VidcDrvClientTest::EvtCb,
                                 this));
    }
    ...
    static VideoFrameDescriptor_t MakeFrame(uint64_t handle,
                                            void*    addr,
                                            uint32_t size,
                                            uint32_t offset = 0,
                                            uint64_t ts_ns  = 0,
...
TEST_F(VidcDrvClientTest, OpenDriver_FailsIfDeviceOpenReturnsNull)
{
    ...
    auto rc = dut.OpenDriver(&VidcDrvClientTest::InDoneCb,
                             &VidcDrvClientTest::OutDoneCb,
                             &VidcDrvClientTest::EvtCb,
                             this);
...
TEST_F(VidcDrvClientTest, SetBufferAndEmptyBuffer_InputPath_ThenInputDoneCallback)
{
    ...
    auto frame = MakeFrame(/*handle*/0x1001, /*addr*/reinterpret_cast<void*>(0xDEAD0001), /*size*/2048);
...
TEST_F(VidcDrvClientTest, SetBufferAndFillBuffer_OutputPath_ThenOutputDoneCallbackWithMetadata)
{
    ...
    auto frame = MakeFrame(/*handle*/0x2001, /*addr*/reinterpret_cast<void*>(0xBEEF0002), /*size*/4096);
...
TEST_F( VidcDrvClientTest, NegotiateBufferReq_DriverRequestsSame_Succeeds )
{
    ...
    EXPECT_EQ( QC_STATUS_OK,
               dut.NegotiateBufferReq( VIDEO_CODEC_BUF_INPUT, bufNum, bufSize ) );
    EXPECT_EQ( 4u, bufNum );
...
TEST_F( VidcDrvClientTest, NegotiateBufferReq_GetPropertyFails_ReturnsError )
{
    ...
    uint32_t bufNum = 4, bufSize = 0;
    EXPECT_EQ( QC_STATUS_FAIL,
               dut.NegotiateBufferReq( VIDEO_CODEC_BUF_OUTPUT, bufNum, bufSize ) );
}
...
TEST_F( VidcDrvClientTest, SetBuffer_Output_Succeeds )
{
    uint8_t b1[512] = {}, b2[512] = {};
    VideoFrameDescriptor_t fd1{}, fd2{};
    fd1.pBuf = b1; fd1.dmaHandle = 0xB001; fd1.size = sizeof( b1 );
    fd2.pBuf = b2; fd2.dmaHandle = 0xB002; fd2.size = sizeof( b2 );
...
TEST_F( VidcDrvClientTest, SetBuffer_IoctlFails_ReturnsError )
{
    ...
    uint8_t buf[512] = {};
    VideoFrameDescriptor_t fd{};
    fd.pBuf = buf; fd.dmaHandle = 0xC001; fd.size = sizeof( buf );
...
TEST_F( VidcDrvClientTest, FreeBuffers_Input_Succeeds )
{
    uint8_t buf[512] = {};
    VideoFrameDescriptor_t fd{};
    fd.pBuf = buf; fd.dmaHandle = 0xD001; fd.size = sizeof( buf );
...
TEST_F( VidcDrvClientTest, FreeBuffers_Output_Succeeds )
{
    uint8_t buf[512] = {};
    VideoFrameDescriptor_t fd{};
    fd.pBuf = buf; fd.dmaHandle = 0xE001; fd.size = sizeof( buf );
...
TEST( VidcDrvClientStatic, OpenDriver_DeviceOpenFails_ReturnsError )
{
    ...
    VidcNodeBase_Config_t cfg{};
    cfg.width = 176; cfg.height = 144; cfg.frameRate = 30;
    cfg.numInputBufferReq = 4; cfg.numOutputBufferReq = 4;
    cfg.bInputDynamicMode = true; cfg.bOutputDynamicMode = true;
...
TEST_F( VidcDrvClientTest, SetDrvProperty_IoctlFails_ReturnsError )
{
    ...
    uint8_t data[64] = {};
    EXPECT_EQ( QC_STATUS_FAIL, dut.SetDrvProperty( VIDC_I_SESSION_CODEC, sizeof(data), data[0] ) );
}
...
TEST_F( VidcDrvClientTest, GetDrvProperty_IoctlFails_ReturnsError )
{
    ...
    uint8_t data[64] = {};
    EXPECT_EQ( QC_STATUS_FAIL, dut.GetDrvProperty( VIDC_I_SESSION_CODEC, sizeof(data), data[0] ) );
}
