...
{
public:
    TestableVidcNodeConfigIfs( Logger &logger )
...
public:
    ...
    const QCNodeMonitoringBase_t& Get( )
    {
        return m_config;
    }
    ...
    QCStatus_e Place( void *ptr, uint32_t &size )
    {
        return QC_STATUS_UNSUPPORTED;
    }
...
public:
    ...
    void WireRealClient(const std::string& name,
                        Logger_Level_e    level,
                        VideoEncDecType_e type,
                        const VidcNodeBase_Config_t& cfg,
                        const VidcCodecMeta_t& meta)
    {
        m_drvClient.Init(name, level, type, cfg); // sets m_bufNum[], type
        ASSERT_EQ(QC_STATUS_OK,
                  m_drvClient.OpenDriver(&TestableVidcNodeBase::InDoneCb,
                                         &TestableVidcNodeBase::OutDoneCb,
                                         &TestableVidcNodeBase::EvtCb,
                                         this)); // registers DeviceCallback
        ASSERT_EQ(QC_STATUS_OK, m_drvClient.InitDriver(meta)); // sets codec, fps, size via SET_PROPERTY
    }
    ...
    std::vector<std::reference_wrapper<VideoFrameDescriptor_t>>& InList() { return m_inputBufferList; }
    std::vector<std::reference_wrapper<VideoFrameDescriptor_t>>& OutList() { return m_outputBufferList; }
    uint32_t* BufSizeArr() { return m_bufSize; }

    QCStatus_e Initialize(QCNodeInit_t&)
    {
        return QC_STATUS_OK;
    }

    QCStatus_e ProcessFrameDescriptor(QCFrameDescriptorNodeIfs&)
    {
        return QC_STATUS_OK;
    }
...
protected:
    ...
    void SetUp() override
    {
        ...
        node.WireRealClient("vidc-node", LOGGER_LEVEL_DEBUG, VIDEO_ENC, m_cfg, m_meta); // encoder path uses RESP_START → RUNNING
    }
...
{
    const VideoCodec_EventType_e evttab[] = {
        VIDEO_CODEC_EVT_FLUSH_INPUT_DONE,
...
TEST_F(VidcNodeBaseTest, Start_Encoder_TransitionsToRunning)
{
    ...
    EXPECT_EQ(QC_OBJECT_STATE_RUNNING, node.GetState()); // EventCallback sets RUNNING on RESP_START
}
...
TEST_F(VidcNodeBaseTest, DeInitialize_TransitionsToInitialAndCloses)
{
    ...
    node.SetState(QC_OBJECT_STATE_READY);
    EXPECT_EQ(QC_STATUS_OK, node.DeInitialize()); // ReleaseResources emits RESP_RELEASE_RESOURCES
    EXPECT_EQ(QC_OBJECT_STATE_INITIAL, node.GetState()); // EventCallback sets INITIAL on RESP_RELEASE_RESOURCES
}

TEST_F(VidcNodeBaseTest, ValidateBuffer_DimensionMismatchIsInvalid)
{
    auto f = MakeFrame(0x1, (void*)0xAAA, 4096, /*w*/640, /*h*/480, m_cfg.inFormat);
    EXPECT_EQ(QC_STATUS_INVALID_BUF, node.ValidateBuffer(f, VIDEO_CODEC_BUF_INPUT)); // checks width/height vs config
}

TEST_F(VidcNodeBaseTest, ValidateBuffer_FormatMismatchForInputIsInvalid)
{
    auto f = MakeFrame(0x1, (void*)0xAAA, 4096, m_cfg.width, m_cfg.height, QC_IMAGE_FORMAT_P010);
    EXPECT_EQ(QC_STATUS_INVALID_BUF, node.ValidateBuffer(f, VIDEO_CODEC_BUF_INPUT)); // compares format to inFormat/outFormat
}
...
TEST_F(VidcNodeBaseTest, ValidateBuffers_DetectsBadDynamicConfig)
{
    ...
    EXPECT_EQ(QC_STATUS_BAD_ARGUMENTS, node.ValidateBuffers()); // logic checks list vs dynamic flags
}
...
TEST_F(VidcNodeBaseTest, AllocateBuffer_NonDynamic_InsufficientDescriptors)
{
    ...
    EXPECT_EQ(QC_STATUS_NOMEM, node.AllocateBuffer(bufs, /*idx*/0, VIDEO_CODEC_BUF_INPUT)); // needs >= numInputBufferReq
}
...
TEST_F(VidcNodeBaseTest, AllocateBuffer_NonDynamic_SizeTooSmall)
{
    ...
    auto f1 = MakeFrame(0x10, (void*)0xB1, /*size*/1024, m_cfg.width, m_cfg.height, m_cfg.inFormat);
    auto f2 = MakeFrame(0x11, (void*)0xB2, /*size*/2048, m_cfg.width, m_cfg.height, m_cfg.inFormat);
    std::vector<std::reference_wrapper<QCBufferDescriptorBase_t>> bufs{
        reinterpret_cast<QCBufferDescriptorBase_t&>(f1),
        reinterpret_cast<QCBufferDescriptorBase_t&>(f2),
    };
    EXPECT_EQ(QC_STATUS_NOMEM, node.AllocateBuffer(bufs, 0, VIDEO_CODEC_BUF_INPUT)); // checks each desc size >= bufSize
}
...
TEST_F(VidcNodeBaseTest, AllocateBuffer_NonDynamic_SliceIsCollected)
{
    ...
    EXPECT_EQ(QC_STATUS_OK, node.AllocateBuffer(bufs, /*idx*/1, VIDEO_CODEC_BUF_INPUT)); // InitBufferForNonDynamicMode picks [1..1+count)
...
TEST_F(VidcNodeBaseTest, NegotiateBufferReq_StoresDriverSize)
{
    ...
    EXPECT_EQ(QC_STATUS_OK, node.NegotiateBufferReq(VIDEO_CODEC_BUF_OUTPUT)); // uses real client Get/Set property flow
...
TEST_F(VidcNodeBaseTest, NegotiateBufferReq_FailsIfDriverNeedsMoreThanAppHas)
{
    ...
    EXPECT_EQ(QC_STATUS_BAD_ARGUMENTS, node.NegotiateBufferReq(VIDEO_CODEC_BUF_INPUT)); // node validates app vs driver counts
}
...
TEST_F(VidcNodeBaseTest, WaitForState_TimesOutWhenStateNeverMatches)
{
    node.SetState(QC_OBJECT_STATE_READY);
    g_sleep_calls = 0;
    EXPECT_EQ(QC_STATUS_TIMEOUT, node.WaitForState(QC_OBJECT_STATE_RUNNING)); // internal timeout after small tick loop
...
TEST_F(VidcNodeBaseTest, GetVidcFormat_Mapping)
{
    ...
    EXPECT_EQ(VIDC_COLOR_FORMAT_UNUSED,    node.GetVidcFormat(QC_IMAGE_FORMAT_COMPRESSED_MAX)); // default branch
}
...
TEST_F(VidcNodeBaseTest, WidthHeightAndFormatChecks)
{
    ...
    VideoFrameDescriptor_t f{};
    f.width = cfg.width; f.height = cfg.height; f.format = cfg.inFormat;

    // OK for input
    EXPECT_EQ(node.ValidateBuffer(f, VIDEO_CODEC_BUF_INPUT), QC_STATUS_OK); // dims+format match

    // Width mismatch
    f.width = cfg.width + 1;
    EXPECT_EQ(node.ValidateBuffer(f, VIDEO_CODEC_BUF_INPUT), QC_STATUS_INVALID_BUF); // mismatch -> INVALID_BUF
    f.width = cfg.width; // restore

    // Input format mismatch
    f.format = QC_IMAGE_FORMAT_P010;
    EXPECT_EQ(node.ValidateBuffer(f, VIDEO_CODEC_BUF_INPUT), QC_STATUS_INVALID_BUF); // input format mismatch

    // Output format mismatch
    f.format = QC_IMAGE_FORMAT_P010;
    EXPECT_EQ(node.ValidateBuffer(f, VIDEO_CODEC_BUF_OUTPUT), QC_STATUS_INVALID_BUF); // output format mismatch
}
...
TEST_F(VidcNodeBaseTest, DynamicVsNonDynamicRules)
{
    ...
    EXPECT_EQ(node1.ValidateBuffers(), QC_STATUS_BAD_ARGUMENTS); // should not provide input in dynamic mode
    node1.InList().clear();
    ...
    EXPECT_EQ(node2.ValidateBuffers(), QC_STATUS_BAD_ARGUMENTS); // should provide input in non-dynamic mode
    ...
    EXPECT_EQ(node3.ValidateBuffers(), QC_STATUS_BAD_ARGUMENTS); // should not provide output in dynamic mode
    node3.OutList().clear();
    ...
    EXPECT_EQ(node4.ValidateBuffers(), QC_STATUS_BAD_ARGUMENTS); // should provide output in non-dynamic mode
}
...
TEST_F(VidcNodeBaseTest, BasicGuards) {
    ...
    EXPECT_EQ(node.ValidateFrameSubmission(f, VIDEO_CODEC_BUF_INPUT, true), QC_STATUS_BAD_STATE); // not running

    node.SetState(QC_OBJECT_STATE_RUNNING);
    f.pBuf = nullptr;
    EXPECT_EQ(node.ValidateFrameSubmission(f, VIDEO_CODEC_BUF_INPUT, true), QC_STATUS_BAD_ARGUMENTS); // null buf

    f.pBuf = reinterpret_cast<void*>(0x1); f.size = 0;
    EXPECT_EQ(node.ValidateFrameSubmission(f, VIDEO_CODEC_BUF_INPUT, true), QC_STATUS_BAD_ARGUMENTS); // zero size required non-zero

    f.size = 16;
    EXPECT_EQ(node.ValidateFrameSubmission(f, VIDEO_CODEC_BUF_OUTPUT, false), QC_STATUS_OK); // zero-size allowed if not required
}
...
TEST_F(VidcNodeBaseTest, OkAndTimeout)
{
    TestableVidcNodeBase node;
    node.SetState(QC_OBJECT_STATE_READY);
    EXPECT_EQ(node.WaitForState(QC_OBJECT_STATE_READY), QC_STATUS_OK); // already in expected state

    node.SetState(QC_OBJECT_STATE_INITIAL);
    EXPECT_EQ(node.WaitForState(QC_OBJECT_STATE_RUNNING), QC_STATUS_TIMEOUT); // times out (1ms threshold in code)
}

TEST_F(VidcNodeBaseTest, InputAndOutput)
{
    TestableVidcNodeBase node; auto cfg = MakeValidConfig();
    ASSERT_EQ(node.Init(cfg), QC_STATUS_OK);
...
TEST_F(VidcNodeBaseTest, NonDynamic_InsufficientAndSizeAndSuccess)
{
    ...
    EXPECT_EQ(node.AllocateBuffer(v, 0, VIDEO_CODEC_BUF_INPUT), QC_STATUS_NOMEM); // insufficient descriptors
    ...
    v2.push_back(c);
    v2.push_back(d);
    EXPECT_EQ(node.AllocateBuffer(v2, 0, VIDEO_CODEC_BUF_INPUT), QC_STATUS_NOMEM); // size too small
    ...
    v3.push_back(y);
    v3.push_back(z);
    EXPECT_EQ(node.AllocateBuffer(v3, 1, VIDEO_CODEC_BUF_INPUT), QC_STATUS_OK); // picks y,z into input list
...
TEST_F(VidcNodeBaseTest, MapsKnownFormatsAndDefault)
{
    ...
    EXPECT_EQ(node.GetVidcFormat(QC_IMAGE_FORMAT_NV12_UBWC), VIDC_COLOR_FORMAT_NV12_UBWC);// UBWC map
    EXPECT_EQ(node.GetVidcFormat(QC_IMAGE_FORMAT_P010), VIDC_COLOR_FORMAT_NV12_P010); // P010 map
    EXPECT_EQ(node.GetVidcFormat(QC_IMAGE_FORMAT_MAX), VIDC_COLOR_FORMAT_UNUSED); // default/unsupported
}
...
TEST_F(VidcNodeBaseTest, ValidAndInvalidStaticConfig)
{
    ...
    EXPECT_EQ(cfgIfs.VerifyAndSet(dataTree, dataTree.Dump(), errors, out), QC_STATUS_OK); // happy path logs/assigns fields
    ...
    errors.clear();
    EXPECT_EQ(cfgIfs.VerifyAndSet(dataTree, dataTree.Dump(), errors, out), QC_STATUS_BAD_ARGUMENTS);
}
